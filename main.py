...
class RamanDataDigitizer:
    ...
    def setup_ui(self):
        ...
        calib_button_row2 = tk.Frame(calib_frame, bg=self.colors['bg_panel'])
        calib_button_row2.pack(fill=tk.X, padx=8, pady=(0, 6))
        tk.Button(calib_button_row2, text="Manual", command=self.enter_manual_calibrate_mode,
                  bg=self.colors['info'], fg='white', font=('Segoe UI', 8, 'bold'),
                  relief=tk.FLAT, bd=0).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))
        tk.Button(calib_button_row2, text="Clear", command=self.clear_calibration,
                  bg=self.colors['danger'], fg='white', font=('Segoe UI', 8, 'bold'),
                  relief=tk.FLAT, bd=0).pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(2, 0))
    ...
    def extract_data_method2(self):
        ...
        try:
            if self.line_masking.detected_lines:
                ...
                tolerance = self.color_tolerance_var.get() if hasattr(self, 'color_tolerance_var') else 5
