...
class LineMaskingSystem:
    ...
    def _separate_lines_clustering(self, all_line_points):
        """Separate lines using DBSCAN-like clustering."""
        if not all_line_points:
            return []

        points_array = np.array(all_line_points, dtype=np.float32)
        ...
        for i in range(len(points_array)):
            ...
            while queue:
                ...
                expansion_indices = np.where(distances <= eps)[0]
...
class MovableStatisticsPanel:
    ...
    def show_panel(self):
        ...
        # Close button
        close_btn = tk.Button(title_bar, text="X", bg='#DC3545', fg='white',
                              font=('Arial', 10, 'bold'), bd=0, width=3,
                              command=self.hide_panel)
        close_btn.pack(side=tk.RIGHT, padx=5, pady=2)

        # Dock button
        self.dock_btn = tk.Button(title_bar, text="Dock", bg='#28a745', fg='white',
                                  font=('Arial', 10, 'bold'), bd=0, width=3,
    ...
    def dock_to_parent(self):
        ...
        self.is_docked = True
        self.dock_btn.config(text="Dock", bg='#ffc107')  # Change icon when docked
    ...
    def toggle_dock(self):
        if self.is_docked:
            ...
            self.is_docked = False
            self.dock_btn.config(text="Dock", bg='#28a745')
        else:
            self.dock_to_parent()
...
class RamanDataDigitizer:
        ...
        try:
            ...
            if exclusion_mask is not None:
                self.debug_label.config(
                    text=f"Debug: Simple detection using {len(self.exclusion_zones)} exclusion zones...")
                self.root.update_idletasks()
            ...
            # FIXED: Use adaptive thresholding to better handle varying backgrounds and line intensities.
            # This is much more effective at picking out the dark lines without grabbing the background.
            binary = cv2.adaptiveThreshold(gray, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
    ...
    def auto_exclude_borders(self):
        ...
        Enhanced to also mask areas outside borders and detect enclosed legends/annotations.
        """
        if self.image is None:
            messagebox.showwarning("No Image", "Please load an image first.")
            return
        ...
        try:
            print("Starting enhanced automatic border detection...")
            ...
            # Method 1: Strong edge detection for borders and legends
            try:
                ...
                # Find contours of potential borders and enclosed areas
                ...
                for contour in contours:
                    ...
                    # Large rectangles are likely panel borders
                    if (area > 10000 and  # Large enough to be a panel
                            w > 100 and h > 100 and  # Reasonable minimum dimensions
                            area < 0.8 * self.image.width * self.image.height and  # Not the entire image
                            len(approx) >= 4):  # Roughly rectangular

                        border_thickness = 8  # Pixels to exclude around the border

                        # Create border exclusion zones (the border lines themselves)
                        # Top border
                        ...
                        # NEW: Create extended border masks to image edges
                        # Mask everything outside the detected panel border

                        # Left extension (from image left edge to border left)
                        if x > 10:  # Only if there's significant space
                            detected_borders.append((
                                0, 0, x, self.image.height
                            ))

                        # Right extension (from border right to image right edge)
                        if (x + w) < (self.image.width - 10):  # Only if there's significant space
                            detected_borders.append((
                                x + w, 0, self.image.width, self.image.height
                            ))

                        # Top extension (from image top to border top)
                        if y > 10:  # Only if there's significant space
                            detected_borders.append((
                                0, 0, self.image.width, y
                            ))

                        # Bottom extension (from border bottom to image bottom)
                        if (y + h) < (self.image.height - 10):  # Only if there's significant space
                            detected_borders.append((
                                0, y + h, self.image.width, self.image.height
                            ))

                    # NEW: Detect smaller enclosed rectangles (legends, annotations)
                    elif (500 < area < 10000 and  # Medium size - likely legends/annotations
                          w > 30 and h > 20 and  # Minimum size for text boxes
                          w < 400 and h < 300 and  # Not too large
                          len(approx) >= 4):  # Roughly rectangular

                        # Check if this is likely a text/legend box by analyzing content
                        roi = gray[y:y + h, x:x + w]

                        # Use adaptive threshold to find text-like content
                        try:
                            binary_roi = cv2.adaptiveThreshold(roi, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
                                                               cv2.THRESH_BINARY_INV, 11, 2)
                            text_pixels = np.sum(binary_roi > 0)
                            total_pixels = roi.size
                            text_density = text_pixels / total_pixels

                            # If there's a reasonable amount of "text-like" content, treat as legend
                            if 0.05 < text_density < 0.7:  # Not too sparse, not too dense
                                # Add padding around the detected legend
                                padding = 10
                                legend_x0 = max(0, x - padding)
                                legend_y0 = max(0, y - padding)
                                legend_x1 = min(self.image.width, x + w + padding)
                                legend_y1 = min(self.image.height, y + h + padding)

                                detected_legends.append((legend_x0, legend_y0, legend_x1, legend_y1))

                        except Exception as e:
                            print(f"Error analyzing potential legend: {e}")
                            # If analysis fails, still add it as a potential legend with basic criteria
                            if area > 1000:  # Only larger ones if we can't analyze content
                                padding = 10
                                legend_x0 = max(0, x - padding)
                                legend_y0 = max(0, y - padding)
                                legend_x1 = min(self.image.width, x + w + padding)
                ...
                print(
                    f"Method 1 detected {len(detected_borders)} border segments and {len(detected_legends)} potential legends")

            except Exception as e:
                print(f"Border detection method 1 failed: {e}")

            # Method 2: Line detection for straight borders (enhanced)
            try:
                ...
                if lines is not None:
                    ...
                    for rho, theta in horizontal_lines[:10]:  # Limit to avoid too many
                        ...
                        if 0 < y_line < self.image.height:
                            ...
                            # NEW: Extended exclusion zones
                            # If line is in upper part of image, mask everything above it
                            if y_line < self.image.height * 0.3:
                                detected_borders.append((
                                    0, 0, self.image.width, y_line - border_thickness
                                ))

                            # If line is in lower part of image, mask everything below it
                            elif y_line > self.image.height * 0.7:
                                detected_borders.append((
                                    0, y_line + border_thickness, self.image.width, self.image.height
                                ))
                    ...
                    for rho, theta in vertical_lines[:10]:  # Limit to avoid too many
                        ...
                        if 0 < x_line < self.image.width:
                            ...
                            # NEW: Extended exclusion zones
                            # If line is on left side of image, mask everything to its left
                            if x_line < self.image.width * 0.3:
                                detected_borders.append((
                                    0, 0, x_line - border_thickness, self.image.height
                                ))

                            # If line is on right side of image, mask everything to its right
                            elif x_line > self.image.width * 0.7:
                                detected_borders.append((
                                    x_line + border_thickness, 0, self.image.width, self.image.height
                                ))
            ...
            all_detected_regions = detected_borders + detected_legends

            if not all_detected_regions:
                messagebox.showinfo("No Borders", "No clear panel borders or enclosed regions detected.\n" +
                                    "This works best with images that have distinct rectangular frames around each graph panel.")
                return

            # Remove overlapping regions to avoid redundancy
            filtered_regions = self.remove_overlapping_regions(all_detected_regions)

            # Add detected regions to existing exclusion zones (don't replace, just extend)
            initial_count = len(self.exclusion_zones)
            self.exclusion_zones.extend(filtered_regions)

            # Redraw to show the new exclusion zones
            self.draw_exclusion_zones()

            # Create detailed success message
            border_count = len(detected_borders)
            legend_count = len(detected_legends)
            total_added = len(filtered_regions)

            message = f"Enhanced border exclusion complete!\n\n"
            message += f"Added {border_count} border exclusions\n"
            message += f"Added {legend_count} legend/annotation exclusions\n"
            message += f"Total new exclusions: {total_added}\n"
            message += f"Total exclusion zones: {len(self.exclusion_zones)}\n\n"
            message += f"Border exclusions now extend to image edges and include enclosed legends."

            messagebox.showinfo("Enhanced Border Exclusion Complete", message)
    ...
    def update_zoom_preview(self):
        ...
        for (x0, y0, x1, y1) in self.exclusion_zones:
            ...
            if clipped_x1 > clipped_x0 and clipped_y1 > clipped_y0:
                zoom_x0 = (clipped_x0 - x_start) * (150 / size);
                zoom_y0 = (clipped_y0 - y_start) * (150 / size)
                zoom_x1 = (clipped_x1 - x_start) * (150 / size);
                zoom_y1 = (clipped_y1 - y_start) * (150 / size)
        ...
        if self.extracted_points:
            ...
            for px, py in self.extracted_points:
                if x_start <= px < x_end and y_start <= py < y_end:
                    zoom_x = (px - x_start) * (150 / size);
                    zoom_y = (py - y_start) * (150 / size)
        ...
        self.zoom_canvas.delete("all")
        self.zoom_canvas.create_image(40, 40, anchor=tk.CENTER, image=self.zoom_photo)
    ...
    def draw_line_in_zoom(self, draw, line_coords, color, size, x_start, y_start, zoom_size=150):
        ...
        zx1 = (x1 - x_start) * (zoom_size / size);
        zy1 = (y1 - y_start) * (zoom_size / size)
        zx2 = (x2 - x_start) * (zoom_size / size);
        zy2 = (y2 - y_start) * (zoom_size / size)
    ...
    def extract_data_method1(self):
        ...
        try:
            if self.selected_color is None:
                messagebox.showwarning("No Color", "Please pick a color for the data line first.")
                return []
            ...
            # Apply line selection mask if available
            line_selection_mask = self.line_masking.get_current_line_mask()
            if line_selection_mask is not None:
                # The final mask is the intersection of the color mask and the selected line's mask
                mask = cv2.bitwise_and(mask, line_selection_mask)
    ...
    def extract_data_method2(self):
        """Advanced method with thickness filtering and corrected line masking support."""
        try:
            if self.selected_color is None and not self.line_masking.detected_lines:
                messagebox.showwarning("Input Needed",
                                       "Please pick a line color or use the line detection tools first.")
                return []

            # --- MASKING LOGIC REWORKED ---

            # 1. Start with a mask of the calibrated plot area
            ...
            if len(self.calibration_points) == 4:
                ...
                # Erode the mask slightly to avoid capturing axis lines
                kernel = np.ones((3, 3), np.uint8)
                plot_mask = cv2.erode(plot_mask, kernel, iterations=2)
            else:
                # If not calibrated, use the whole image area as a fallback
                plot_mask.fill(255)

            # 2. Create the color mask based on user selection, if a color is picked
            color_mask = None
            if self.selected_color:
                ...
                bgr_image = cv2.cvtColor(self.image_array, cv2.COLOR_RGB_BGR)
                color_mask = cv2.inRange(bgr_image, lower_bound, upper_bound)

            # 3. Get the line selection mask, if one is active
            line_selection_mask = self.line_masking.get_current_line_mask()

            # 4. Combine the masks to create the final mask for digitization
            # Start with the calibrated plot area
            final_mask = plot_mask

            # If a line has been detected and selected, use it as a boundary.
            if line_selection_mask is not None:
                final_mask = cv2.bitwise_and(final_mask, line_selection_mask)

            # If a color has been selected, apply the color mask.
            if color_mask is not None:
                final_mask = cv2.bitwise_and(final_mask, color_mask)

            # --- END OF MASKING LOGIC REWORK ---
    ...
    def extract_data_dual(self):
        ...
        if not all([self.image, len(self.calibration_points) == 4]):
            ...
            messagebox.showwarning("Prerequisites Missing", message)
            return

        # Check that either a color is selected or a line is detected
        if self.selected_color is None and not self.line_masking.detected_lines:
            messagebox.showwarning("Prerequisites Missing",
                                   "Please ensure:\n• A line color is selected OR\n• A line has been detected with the masking tools")
            return
