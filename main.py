...
class RamanDataDigitizer:
    ...
    def setup_ui(self):
        ...
        self.color_tolerance_var = tk.IntVar(value=5)
        tk.Spinbox(tolerance_frame, from_=1, to=60, textvariable=self.color_tolerance_var,
                   width=4, font=('Segoe UI', 8), relief=tk.FLAT, bd=1,
                   bg=self.colors['bg_secondary']).pack(side=tk.RIGHT)
        self.tolerance_var = self.color_tolerance_var  # legacy compatibility

        toggle_frame = tk.Frame(color_frame, bg=self.colors['bg_panel'])
        toggle_frame.pack(fill=tk.X, pady=(0, 6), padx=8)
        tk.Checkbutton(toggle_frame, text="Use adaptive interpolation",
                       variable=self.use_interpolation,
                       bg=self.colors['bg_panel'], fg=self.colors['text_secondary'],
                       selectcolor=self.colors['bg_secondary'],
                       activebackground=self.colors['bg_panel'],
                       activeforeground=self.colors['text_primary']).pack(anchor='w')
        ...
        tk.Button(detect_frame, text="Simple Line Isolation", command=self.simple_line_detection,
                  bg=self.colors['warning'], fg='white', font=('Segoe UI', 9, 'bold'),
                  relief=tk.FLAT, bd=0).pack(fill=tk.X, pady=2)
        tk.Button(detect_frame, text="Advanced Line Isolation", command=self.detect_lines,
                  bg=self.colors['primary'], fg='white', font=('Segoe UI', 8, 'bold'),
                  relief=tk.FLAT, bd=0).pack(fill=tk.X, pady=2)
        ...
        tk.Button(action_button_frame, text="Dual Extract",
                  ...
                  relief=tk.FLAT, bd=0).pack(fill=tk.X, pady=2)
        tk.Button(action_button_frame, text="Advanced Extraction",
                  command=self.extract_data_method2,
        ...
        tk.Button(action_button_frame, text="Manual Points Mode",
                  ...
                  relief=tk.FLAT, bd=0).pack(fill=tk.X, pady=2)
        tk.Button(action_button_frame, text="Clear Data Points",
                  command=self.clear_data_points,
    ...
    def extract_data_method1(self):
        ...
        try:
            self.color_tolerance = int(self.color_tolerance_var.get())
    ...
    def extract_data_dual(self):
        ...
        if not all([self.image, len(self.calibration_points) == 4]):
            ...
            if not self.selected_color and not self.line_masking.detected_lines: missing.append(
                "A line color is selected OR lines are detected")
    ...
    def clear_data_points(self):
        """Public helper wired to the UI button to clear extracted data."""
        self.clear_data()
        self.redraw_overlays()
